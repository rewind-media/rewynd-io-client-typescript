...
import * as spec from "@rewynd.io/rewynd-io-spec";
...

{
  (async () => {
    ...
    // const schemaGen = (await import("openapi-typescript")).default;

    if (typeof output != "string") {
      throw "Must have output path as argument.";
    }

    // const schema = await schemaGen(spec, {
    //   immutableTypes: true,
    //   pathParamsAsTypes: true,
    // });
    // await fs.writeFile(Path.resolve(output, "schema.ts"), schema);
